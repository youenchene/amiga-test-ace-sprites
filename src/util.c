#include <ace/utils/bitmap.h>
#include <ace/managers/log.h>


// Lookup Table to invert bits
static const UBYTE bitReverseTable[256] = {
  
};

tBitMap *xFlipInterleavedBitmap(tBitMap *pBitMap) {
  tBitMap *pSrc = pBitMap;
  UBYTE ubRowSize=bitmapGetByteWidth(pSrc);
  UBYTE ubByteRowSize=pSrc->BytesPerRow;
  UWORD uwHeight = pSrc->Rows;
  UWORD uwWidth = ubRowSize*8;
  UWORD uwDepth = pSrc->Depth;
  tBitMap *pDest = bitmapCreate(uwWidth, uwHeight, uwDepth, BMF_CLEAR | BMF_INTERLEAVED);
  for (UWORD y = 0; y < uwHeight; y++) {
    for (UBYTE bitplane = 0; bitplane < uwDepth ; bitplane++) {
      UBYTE *pSrcPlane = pSrc->Planes[bitplane];
      UBYTE *pDestPlane = pDest->Planes[bitplane];
        for (UWORD x = 0; x < ubRowSize; x++) {
          pDestPlane[y * ubByteRowSize + x] =  bitReverseTable[pSrcPlane[y * ubByteRowSize + (ubRowSize - 1 - x)]];
        }
      }
    }
    return pDest;
}
